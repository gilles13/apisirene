...
#' @examples trymynaflib()
#' @export
trymynaflib <- function() {
	system2("clear")
	message("Nous allons tenter de trouver le code naf ensemble !")
	message("Dites moi ce que vous cherchez comme activité ?")
	var1 = readline(prompt = "Entrer une activité : ")
	if(length(var1) == 0) stop("Il faut saisir qque chose ^^'")
	res <- 
		nafniv5 |> 
		slice(grep(pattern = var1, x = Libellé, ignore.case = TRUE))
	if(nrow(res) == 0) {
		message("Désolé, je n'ai rien trouvé ... :(")
		message("Ré-essayer avec un expression régulière ?")
	} else return(res)
}
...
trymycjlib <- function() {
	...
	message("Dites moi ce que vous cherchez comme catégorie juridique ?")
...
#' @description Affiche une page html synthétisant de l'information de base sur un etablissement
#' @param siret une chaine de caractere pour l'etablissement a consulter
